...
def reverseCommulitive(df,column_name):
    ...
    dic = {column_name: 'commutive '+column_name}
...
def upDateCountryDate(country,date, df_cofermed,df_deathes,df_recovered):

    d2 = dddt.datetime.strptime(str(date), '%Y-%m-%d').strftime('%#m/%#d/%Y')
    ...
    commultive_confirmed=reverseCommulitive(df_cofermed[date_range_formated:],'confirmed')['commutive confirmed'].tolist()

    deathes = reverseCommulitive(df_deathes[date_range_formated:], 'deaths')['deaths'].tolist()
    commultive_deathes = reverseCommulitive(df_deathes[date_range_formated:], 'deaths')['commutive deaths'].tolist()

    recovered = reverseCommulitive(df_recovered[date_range_formated:], 'recovered')['recovered'].tolist()
    commultive_recovered = reverseCommulitive(df_recovered[date_range_formated:], 'recovered')['commutive recovered'].tolist()

    print(confermed)
    print(commultive_confirmed)

    for i in range (len(confermed)):
        date += timedelta(days=1)
        print(commultive_confirmed[i])
        print(confermed[i])

        print(commultive_recovered[i])
        print(recovered[i])

        print(commultive_deathes[i])
        print(deathes[i])

        cursor = connection.cursor()
        cursor.execute("INSERT INTO  public.\"COV19Predictor_uk\" VALUES  (DEFAULT ,%(date)s ,%(cumulative_confirmed_cases)s,%(confirmed_cases)s,%(cumulative_recovered_cases)s,%(recovered_cases)s,%(cumulative_death_cases)s,%(death_cases)s)",
                       {
                        'date': date,
...
def checkData(request):
    ...
    df_recovred = pd.read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv")
   ...
   # print(isDatabaseUpdateStatus(datetime.strptime(df_confirmed_country.index.tolist()[-1], '%m/%d/%y').date()))
    date = isDatabaseUpdateStatus(datetime.strptime(df_confirmed_country.index.tolist()[-1], '%m/%d/%y').date())
    if date != True:
        print("update now")


        new_date = upDateCountryDate("Egypt",date,df_confirmed_country,df_deathes_country,df_recovred_country)
